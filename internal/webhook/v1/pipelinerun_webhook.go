...
func (d *pipelineRunCustomDefaulter) Default(ctx context.Context, obj runtime.Object) error {
	...
	log.Printf("Setting Default values : %+v", d.configStore.GetConfig())
	if _, exists := plr.Labels[common.QueueLabel]; !exists {
		plr.Labels[common.QueueLabel] = d.configStore.GetConfig().QueueName
	}
	if d.configStore.GetConfig().MultiKueueOverride {
		plr.Spec.ManagedBy = ptr.To(common.ManagedByMultiKueueLabel)
	}
	for _, mutator := range d.configStore.mutators {
